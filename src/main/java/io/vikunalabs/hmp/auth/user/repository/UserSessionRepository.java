...
public interface UserSessionRepository extends JpaRepository<UserSession, Long> {
    ...
    @Modifying
    @Query("UPDATE UserSession s SET s.active = false, s.logoutReason = :reason WHERE s.user.id = :userId AND s.active = true")
    int invalidateAllUserSessions(@Param("userId") Long userId, @Param("reason") LogoutReason reason);

    @Modifying
    @Query("UPDATE UserSession s SET s.active = false, s.logoutReason = 'SESSION_EXPIRED' WHERE s.expiresAt < :now AND s.active = true")
